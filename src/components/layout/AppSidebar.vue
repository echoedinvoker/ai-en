      ...
      <nav class="space-y-2">
        ...
        <div v-for="item in menuItems" :key="item.key" class="space-y-1">
          ...
          <button @click="toggleExpanded(item.key)"
            class="w-full flex items-center justify-between px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-md transition-colors"
          ...
          <div v-show="expandedItems.has(item.key)" class="ml-4 space-y-1">
            <button v-for="child in item.children" :key="child.key" @click="selectChild(item.key, child.key)"
              class="w-full text-left px-3 py-2 text-sm text-gray-600 hover:bg-blue-50 hover:text-blue-700 rounded-md transition-colors"
              :class="{
